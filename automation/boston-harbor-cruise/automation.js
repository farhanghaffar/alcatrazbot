...
async function bostonHarborCruise(bookingData, tries, payload) {
  ...
  try {
    ...
    if (bookingData.tourType === "Boston City Cruise") {
      tourURL =
        "https://www.cityexperiences.com/boston/city-cruises/historic-sightseeing-cruise/";
    }
    ...
    const ticketQuantities = {
      ...
      _booking_child_under_three: bookingData.child_under_three,
    };
    for (const [ticketType, quantity] of Object.entries(ticketQuantities)) {
      if (quantity > 0) {
        ...
        if (ticketType === "_booking_adults") {
          ...
          await incrementTickets(frameHandle, "Child (3-11)", quantity);
        } else if (ticketType === "_booking_child_under_three") {
          await incrementTickets(frameHandle, "Child (Under 3)", quantity);
        }
      }
    }
  ...
  if(errorMsgVisible && orderRejectedErrorVisible) {
    ...
    await sendEmail(
      ...
      ["tickets@bostoncruisetickets.com"], // CC email(s), can be a single email or comma-separated multiple mails
      // [],
    ...
    try {
      await sendEmail(
        ...
        "farhan.qat123@gmail.com", // recipient email address
        ["tickets@bostoncruisetickets.com"], // CC email(s), can be a single email or comma-separated
        // [],
        screenshotPath, // path to the screenshot
...
module.exports = { bostonHarborCruise };
