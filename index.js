...
const {
  BayCruiseTickets,
} = require("./automation/bay-cruise-tickets/automation");
const {
  bostonHarborCruise,
} = require("./automation/boston-harbor-cruise/automation");
const {
  NiagaraCruiseTickets,
} = require("./automation/niagara-cruise-tickets/automation");
const {
  FortSumterTickets,
} = require("./automation/fort-sumter-tickets/automation");
const {
  KennedySpaceCenterTickets,
} = require("./automation/kennedy-space-center-tickets/automation");
...
const {
  BookMckinacIslandTickets,
} = require("./automation/mackinac-sheplers-ferry-tickets/automation");
...
const verifyWooCommerceWebhook = (req, res, next) => {
  ...
  const secret =
    ...
    "z6>9w P`}@5^9A~X,sX)-Qw)B< Z#e@j4xbCp:7hW-z{fl{ ?]";
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    reqBody?.line_items[0]?.meta_data.forEach((item) => {
      switch (item.key) {
        ...
        case "Booking Time":
          ...
          break;
        case "Person Names":
          orderData.personNames = item?.value
            .split(", ")
            .map((name) => name.trim());
          break;
    ...
    let tries = 0;
    const maxRetries = 3;
    let bookingResult = await statueTicketingBookTour(
      orderData,
      tries,
      reqBody
    );
    ...
    while (
      ...
      !bookingResult?.error?.includes("Payment not completed") &&
      !bookingResult?.error?.includes("Expected format is MM/YY.") &&
      !bookingResult?.error?.includes("Month should be between 1 and 12.") &&
      !bookingResult?.error?.includes("The card has expired.") &&
      !bookingResult?.error?.includes("Order Rejected") &&
      !bookingResult?.error?.includes(
...
app.post("/charge", async (req, res) => {
  try {
    ...
    if (statementDescriptorPrefix.length > maxPrefixLength) {
      statementDescriptorPrefix = statementDescriptorPrefix.slice(
        0,
        maxPrefixLength
      );
    }
  ...
  try {
    ...
    const orderDetails = {
      ...
      sChargesAmount: reqBody?.line_items[0]?.meta_data
        .find(
          (item) =>
            item.key === "Service Charges" ||
            item.key === "_booking_serviceCharges"
        )
        ?.value.replace(/[^0-9.-]+/g, ""),
      siteName: "",
      ...
      cardNumber:
        reqBody?.meta_data.find(
          (item) => item.key.toLowerCase() === "credit_card_number"
        )?.value || "",
      cardExpiryDate:
        reqBody?.meta_data.find(
          (item) => item.key.toLowerCase() === "credit_card_expiration"
        )?.value || "",
      cardCVC:
        reqBody?.meta_data.find(
          (item) => item.key.toLowerCase() === "credit_card_cvc"
        )?.value || "",
    ...
    if (reqBody?.line_items[0]?.name === "Boston Cruise Tickets") {
      ...
      reqBody?.line_items[0]?.name === "Statue of Liberty Reservation"
    ) {
      ...
      reqBody?.line_items[0]?.name === "Kennedy Space Center Tickets"
    ) {
    ...
    setImmediate(async () => {
      try {
        ...
        if (bookingResult.success) {
          ...
          console.error(
            "Potomac Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
...
app.post("/bay-cruise-tickets-webhook", async (req, res) => {
  ...
  try {
      ...
      switch (item.key) {
        ...
        default:
          ...
          if (item.key.toLowerCase() === "child (ages 5-11) quantity") {
            ...
            item.key.toLowerCase() === "junior (ages 12-17) quantity"
          ) {
            orderData.juniors = parseInt(item.value, 10);
    ...
    console.log(
      "Bay Cruises Tickets: After manipulation, data is: ",
      orderData
    );
    ...
    res.status(200).json({
      message:
        "Bay Cruises Tickets: Webhook received. Processing in background.",
    });
    ...
    setImmediate(async () => {
      try {
        ...
        if (bookingResult.success) {
          console.log(
            "Bay Cruises Tickets: Booking automation completed successfully"
          );
        } else {
          console.error(
            "Bay Cruises Tickets: Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
        console.error(
          "Bay Cruises Tickets: Error in booking automation:",
          automationError
        );
      }
    });
...
app.post("/boston-harbor-cruise-tickets-webhook", async (req, res) => {
  ...
  try {
    ...
    console.log(
      "Boston Harbor Cruise: After manipulation, data is: ",
      orderData
    );
    ...
    res.status(200).json({
      message:
        "Boston Harbor Cruise Tickets Webhook received. Processing in background.",
    });

    setImmediate(async () => {
      try {
        console.log(
          "Starting Boston harbor cruise booking automation process..."
        );
        let tries = 0;
        const maxRetries = 3;
        ...
        if (bookingResult.success) {
          console.log(
            "Boston Harbar Cruise Booking automation completed successfully"
          );
        } else {
          console.error(
            "Boston Harbar Cruise Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
        console.error(
          "Error in Boston Harbar Cruise booking automation:",
          automationError
        );
      }
    });
...
app.post("/niagara-cruise-tickets-webhook", async (req, res) => {
  ...
  try {
      ...
      switch (item.key) {
        ...
        default:
          ...
          if (item.key.toLowerCase() === "child (3 - 12)  quantity") {
            ...
            item.key.toLowerCase() === "infant (2 and under) quantity"
          ) {
    ...
    res.status(200).json({
      message:
        "Niagara Cruise Tickets Webhook received. Processing in background.",
    });
    ...
    setImmediate(async () => {
      try {
        ...
        let tries = 0;
        const maxRetries = 3;
        let bookingResult = await NiagaraCruiseTickets(
          orderData,
          tries,
          reqBody
        );
        ...
        if (bookingResult.success) {
          console.log(
            "Niagara Cruise Booking automation completed successfully"
          );
        } else {
          console.error(
            "Niagara Cruise Cruise Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
        console.error(
          "Error in Niagara Cruise booking automation:",
          automationError
        );
      }
    });
...
app.post("/fort-sumter-ticketing-webhook", async (req, res) => {
  ...
  try {
      ...
      switch (item.key) {
        ...
        default:
          ...
          if (item.key.toLowerCase() === "children (4-11) quantity") {
            ...
            "senior/military (62 & older/active military) quantity"
          ) {
            orderData.senior_military = parseInt(item.value, 10);
          } else if (
            item.key.toLowerCase() === "infants (ages 3 & younger) quantity"
          ) {
    ...
    console.log(
      "Fort Sumter Tickets: After manipulation, data is: ",
      orderData
    );
    ...
    res.status(200).json({
      message:
        "Fort Sumter Tickets: Webhook received. Processing in background.",
    });
    ...
    setImmediate(async () => {
      try {
        ...
        if (bookingResult.success) {
          console.log(
            "Fort Sumter Tickets: Booking automation completed successfully"
          );
        } else {
          console.error(
            "Fort Sumter Tickets: Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
        console.error(
          "Fort Sumter Tickets: Error in booking automation:",
          automationError
        );
      }
    });
...
app.post("/kennedy-space-center-ticketing-webhook", async (req, res) => {
  console.log(
    "Kennedy SpaceCenter Tickets: Order data:",
  ...
  try {
    ...
    console.log(
      "Kennedy SpaceCenter Tickets: After manipulation, data is: ",
      orderData
    );
    ...
    res.status(200).json({
      message:
        "Kennedy SpaceCenter Tickets: Webhook received. Processing in background.",
    });
    ...
    setImmediate(async () => {
      try {
        ...
        if (bookingResult.success) {
          console.log(
            "Kennedy SpaceCenter Tickets: Booking automation completed successfully"
          );
        } else {
          console.error(
            "Kennedy SpaceCenter Tickets: Booking automation failed:",
            bookingResult.error
          );
        }
      } catch (automationError) {
        console.error(
          "Kennedy SpaceCenter Tickets: Error in booking automation:",
          automationError
        );
      }
    });
